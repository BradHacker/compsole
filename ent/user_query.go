...
// WithUserToTeam tells the query-builder to eager-load the nodes that are connected to
// the "UserToTeam" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithUserToTeam(opts ...func(*TeamQuery)) *UserQuery {
	query := &TeamQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withUserToTeam = query
	return uq
}

// WithUserToToken tells the query-builder to eager-load the nodes that are connected to
// the "UserToToken" edge. The optional arguments are used to configure the query builder of the edge.
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withUserToTeam; query != nil {
	...
	if query := uq.withUserToToken; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[uuid.UUID]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.UserToToken = []*Token{}
		}
		query.withFKs = true
		query.Where(predicate.Token(func(s *sql.Selector) {
			s.Where(sql.InValues(user.UserToTokenColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.user_user_to_token
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_user_to_token" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {

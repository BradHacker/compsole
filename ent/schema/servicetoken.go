...
func (ServiceToken) Fields() []ent.Field {
	return []ent.Field{
		...
		field.UUID("refresh_token", uuid.UUID{}).Comment("[REQUIRED] The refresh token used to renew an expired service account session. These are only valid for 1 hour after the associated token expires."),

...
func (f ServiceAccountFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.ServiceAccountMutation)
	if !ok {
...
// The VmObjectFunc type is an adapter to allow the use of ordinary
// function as VmObject mutator.
type VmObjectFunc func(context.Context, *ent.VmObjectMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f VmObjectFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.VmObjectMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.VmObjectMutation", m)
	}
	return f(ctx, mv)
}
